...
function extractColumnNames(req) {
  var extractedColumnNames = [];
  
  for (index in req.query.columns) {
    extractedColumnNames.push(req.query.columns[index].name);
  }
...
function initializeMongoDataTables(connectionUri, collectionName) {
  function handleRequest(req, res, next) {
    ...
    function queryData(db, callback) {
      var extractedColumnNames = extractColumnNames(req);
